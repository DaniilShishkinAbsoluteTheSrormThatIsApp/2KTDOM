// Массив с информацией о котиках
const cats = [
    {
        "name": "Лара",
  ...
  function displayKittens() {
    var kittensContainer = document.getElementById("kittens-container");
  
    cats.forEach(function(cat) {
      var kittenCard = document.createElement("div");
      kittenCard.className = "kitten-card";
  
      var kittenImage = document.createElement("img");
      kittenImage.className = "kitten-image";
      kittenImage.src = cat.img_link;
      kittenImage.alt = cat.name;
  
      var kittenInfo = document.createElement("div");
      kittenInfo.className = "kitten-info";
  
      var kittenName = document.createElement("h2");
      kittenName.className = "kitten-name";
      kittenName.textContent = cat.name;
  
      var kittenAgeRate = document.createElement("div");
      kittenAgeRate.className = "kitten-age-rate";
  
      var kittenAge = document.createElement("p");
      if (cat.age === 1) {
        kittenAge.textContent = "Возраст: " + cat.age + " год";
    } else if (cat.age > 1 && cat.age < 5) {
        kittenAge.textContent = "Возраст: " + cat.age + " года";
    } else {
        kittenAge.textContent = "Возраст: " + cat.age + " лет";
    }
      var kittenRate = document.createElement("p");
      kittenRate.textContent = "Рейтинг: " + cat.rate + "/10";

      var kittenfavoriteStatus = document.createElement("p");
      kittenfavoriteStatus.className = "kitten-fav";
      kittenfavoriteStatus.textContent = "Избранный: " + (cat.favorite ? "да" : "нет");
      
      kittenAgeRate.appendChild(kittenAge);
      kittenAgeRate.appendChild(kittenRate);
  
      var kittenDescription = document.createElement("p");
      kittenDescription.className = "kitten-description";
      kittenDescription.textContent = cat.description;
  
      kittenInfo.appendChild(kittenName);
      kittenInfo.appendChild(kittenAgeRate);
      kittenInfo.appendChild(kittenDescription);
  
      kittenCard.appendChild(kittenImage);
      kittenCard.appendChild(kittenInfo);
  
      kittensContainer.appendChild(kittenCard);
    });
  }
  
  // Вызываем функцию для отображения котик
  displayKittens()